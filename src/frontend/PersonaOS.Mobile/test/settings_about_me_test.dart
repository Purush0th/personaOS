...
void main() {
  ...
  testWidgets('writes it back when it changed', (tester) async {
    final requests = await run(tester, () async {
      await tester.enterText(find.widgetWithText(TextField, 'Vegetarian.'), 'Vegetarian. Lives in Lisbon.');
      await save(tester);
    });

    final put = requests.singleWhere((r) => r.method == 'PUT' && r.url.path == '/api/profile');
    expect(jsonDecode(put.body), {'aboutMe': 'Vegetarian. Lives in Lisbon.'});
  });
